...
# Taking the lag
for (i in 1:10) {
  excessquarter <- mutate(excessmonth, !!paste("lag_", i, sep = "") := lag(excess_returns, i))
}
# Taking the lag
for (i in 1:10) {
  excessyear <- mutate(excessmonth, !!paste("lag_", i, sep = "") := lag(excess_returns, i))
}
```
...
ts_quarter <- lag_predictors(ts_quarter)
ts_year <- lag_predictors(ts_year)
```
...
ts_quarter <- merge(ts_quarter, excessmonth, by = "time", all.x = TRUE) # Join excessmonth with ts_quarter for the all predictors model
ts_quarter <- ts_quarter[, -grep("excess_returns.y", colnames(ts_quarter))]

# Rename lagged variables
lag_cols <- grep("^lag_", colnames(ts_quarter), value = TRUE)
new_names <- gsub("^lag_", "excess_returns_lag_", lag_cols)
colnames(ts_quarter)[grep("^lag_", colnames(ts_quarter))] <- new_names


# join for year

ts_year <- merge(ts_year, excessmonth, by = "time", all.x = TRUE) # Join with ts_year for the all predictors model
ts_year <- ts_year[, -grep("excess_returns.y", colnames(ts_year))]

# Rename lagged variables
lag_cols <- grep("^lag_", colnames(ts_year), value = TRUE)
new_names <- gsub("^lag_", "excess_returns_lag_", lag_cols)
colnames(ts_year)[grep("^lag_", colnames(ts_year))] <- new_names
```
...
excessquarter <- na.omit(excessquarter)
excessyear <- na.omit(excessyear)
...
### Taking training and testing set
...
validation_data <- subset(excessmonth, time <= excessmonth$time[split_point])
