...
MSFE["Forecast Horizon"] <- rownames(MSFE)
TT_MSFE["Forecast Horizon"] <- rownames(TT_MSFE)

combined_df <- rbind(
  cbind("Approach" = "Rolling Forecast", MSFE),
  cbind("Approach" = "Train Test Split", TT_MSFE)
)

combined_df

combined_df[["Auto-Regressive"]] <- as.numeric(combined_df[["Auto-Regressive"]])
combined_df[["Regression Tree"]] <- as.numeric(combined_df[["Regression Tree"]])
combined_df[["Random Forest"]] <- as.numeric(combined_df[["Random Forest"]])
combined_df[["Forward Stepwise"]] <- as.numeric(combined_df[["Forward Stepwise"]])


combined_df[["Auto-Regressive"]] <- round(combined_df[["Auto-Regressive"]], 3)
combined_df[["Regression Tree"]] <- round(combined_df[["Regression Tree"]], 3)
combined_df[["Random Forest"]] <- round(combined_df[["Random Forest"]], 3)
combined_df[["Forward Stepwise"]] <- round(combined_df[["Forward Stepwise"]], 3)
combined_df
...
# Diebold-Mariano Test
dm.test(
  Mregression_laggedOUT$residuals,
  rfmlOUT$residuals,
  alternative = "two.sided",
  h = 120,
  power = 2,
...
# Diebold-Mariano Test
dm.test(
  Qregression_laggedOUT$residuals,
  rfqlOUT$residuals,
  alternative = "two.sided",
  h = 40,
  power = 2,
...
# Diebold-Mariano Test
dm.test(
  Aregression_laggedOUT$residuals,
  rfalOUT$residuals,
  alternative = "two.sided",
  h = 10,
  power = 2,
...
# Diebold-Mariano Test All Covariates
dm.test(
  arm$residuals,
  Mregression_OUT$residuals,
  alternative = "two.sided",
